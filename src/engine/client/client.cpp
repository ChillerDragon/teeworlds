...
void CClient::ProcessServerPacket(CNetChunk *pPacket)
{
	...
	if(Sys)
	{
		...
		{
			CUnpacker PrintPacker = Unpacker;
			PrintSnapshot(Msg, PrintPacker);
			int NumParts = 1;
			int Part = 0;
...
void CClient::PrintSnapshot(int Msg, CUnpacker &Unpacker)
{
	if(Config()->m_Debug < 2)
		return;
			...
			if(DeltaTick >= 0)
			{
				...
				if(DeltashotSize < 0)
				{
					...
					if(Config()->m_Debug)
					{
						char aBuf[256];
						str_format(aBuf, sizeof(aBuf), "  error, couldn't find the delta snapshot");
						m_pConsole->Print(IConsole::OUTPUT_LEVEL_DEBUG, "network_in", aBuf);
					}
