...
void CStats::OnRender()
{
	...
	for(int j = 0; j < NumPlayers; j++)
	{
		...
		const bool HighlightedLine = apPlayers[j] == m_pClient->m_LocalClientID || (m_pClient->m_Snap.m_SpecInfo.m_Active && apPlayers[j] == m_pClient->m_Snap.m_SpecInfo.m_SpectatorID);
		...
		if(Config()->m_ClStatboardInfos & TC_STATS_WEAPS)
		{
			...
			for(i = 0; i < NUM_WEAPONS; i++)
			{
				extern int _dummy[(int)(NUM_WEAPONS == 6)];
				(void)_dummy; // static assert that there are 6 weapons
